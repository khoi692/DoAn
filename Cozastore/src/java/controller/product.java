...
public class product extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        int number = (size%8==0?(size/8):((size/8)+1)); //number of page
        ...
        List<Product> list = productDAO.pagingProduct(products, numberPerPage, page);
        
        request.setAttribute("colorList", colors);
        request.setAttribute("productList", list);
        session.setAttribute("allProduct", products);
        session.setAttribute("page", page);
        session.setAttribute("number", number); // so trang
